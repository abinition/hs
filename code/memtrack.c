...
char *CopyString(char *InString)
{
  ...
  if(InString != NULL)
  {
    p = (char*) malloc(strlen(InString) + 1);
    if(NULL != p)
    {
      strcpy(p, InString);
    }
                           ...
                           int   LineNumber)
{
  ...
  p = (char*) malloc(Length + sizeof(ALIGN));
  if(0 == ErrorStatus)
  {
...
/* UNCOMMENT THESE TWO LINES PLUS THE
 * DEBUG STATEMENT BELOW AND YOU WILL FIND OUT
 * THE NAMES OF DATA COMPONENTS THAT WERE NOT FREED.
 */
...
{
  gHyp_util_logInfo(
          "%8p allocated %7u byt%s at Line %5d of File %s.",
          p1->Ptr,
  ...
  /* When sDATA objects get left behind, its really hard 
   * to know where, so this helps us get the name of the 
   * sData object
   */
  if ( p1->Size == 60 && strstr ( p1->FileName, "data.c" ) ) {
                ...
                int           LineNumber)
{
  ...
  switch(Msg)
  {
    case MEMTRK_MEMALLOC:
      ...
      {
        gHyp_util_logInfo(
                "ERROR in debugging code - failed to add node to memory tree.\n");
      }
      else
      {
        CurrAlloc += Size;
    ...
    case MEMTRK_MEMFREE:
      ...
      if(NULL != NodePtr)
      {
        ...
        if(CurrAlloc < 0)
        {
          gHyp_util_logInfo( "ERROR: More memory released than allocated!\n");
        }
      ...
      else
      {
         ...
         */
        gHyp_util_logInfo("Attempted to free unallocated block %p at Line %d of File %s.\n",
                EntryBuilder.Ptr,
    ...
    case MEMTRK_REPORT:
      ...
      gHyp_util_logInfo( "Memory Tracker Report\n");
      gHyp_util_logInfo( "---------------------\n\n");
      ...
      if(CurrAlloc == 0)
      {
        gHyp_util_logInfo ( "None! (Well done!)");
      };
      break;
    case MEMTRK_DESTROY:
                    ...
                    PAYLOAD *Payload)
{
  ...
  if(*Node == NULL)
  {
    *Node = (MEMTREE*) malloc(sizeof **Node);
    if(*Node != NULL)
    {
      Twig = *Node;
